...
playstore = playstore.drop_duplicates(subset = ['App'], keep='first', inplace=False, ignore_index=False)
...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.loc[:,['Category', 'App', 'Reviews', 'Rating']].\
            groupby(['Category', 'App'], as_index = False).mean({'Reviews','Rating'}).\
            sort_values('Reviews', ascending = False).head(10).reset_index().\
            loc[:,['Category', 'App', 'Reviews', 'Rating']].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    Paid_App = df2[df2['Type'] == 'Paid'].\
           groupby('Category').agg({'Price': 'mean'}).\
           sort_values('Price', ascending = False).head(10).sort_values('Price', ascending = True)
    X = Paid_App.index
    Y = Paid_App['Price']
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
